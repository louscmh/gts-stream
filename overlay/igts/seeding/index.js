...
(async () => {
    try {
        const jsonData = await $.getJSON("../../../_data/seeding_igts.json");
        jsonData.Teams.map((seed) => {
            seedData.push(seed);
        });
        const jsonData2 = await $.getJSON("../../../_data/quals_data_igts.json");
        jsonData2.map((map) => {
...
class PlayerController {
    ...
    async updateDetails(seedData) {
        ...
        setTimeout(function () {
            ...
            adjustFont(document.getElementById("playerName"), 266, 28);
